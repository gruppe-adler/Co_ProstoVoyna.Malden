/*
	...
	Example:
	_null = [trawler1, "LEFT BACK", 90] execVM "sinkship.sqf";
*/
...
{
  detach _x;
  if (isMultiplayer) then  { _x enableSimulationGlobal true; } else { _x enableSimulation true; };
  _x setDamage 1;
  _x setVelocity [random 2 -1,random 2 -1 ,0.5];
} forEach attachedObjects _ship;

for [{_i=0}, {_i<15}, {_i=_i+1}] do {
	
	_light = "rhs_ammo_nspn_red" createVehicle [getPos trawler select 0, getPos trawler select 1, 1];
	_light setVelocity [random 5 - random 10, random 5 - random 10, 5];
